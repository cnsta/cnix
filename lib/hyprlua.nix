...
{lib}: let
  ...
  toLua = indent: v:
    ...
    then
      (
        ...
        then "{ ${concatMapStringsSep ", " (toLua indent) v} }"
        else "{\n${
    ...
    else throw "toHyprlua: cannot serialize value of unsupported type";
  ...
  wsArg = a:
    if isInt a
    then toString a
    else if builtins.match "[0-9]+" a != null
    then a
    else escapeStr a;
  ...
  mkAction = b:
    ...
    in
      if d == "exec"
      then "hl.dsp.exec_cmd(${escapeStr a})"
      else if d == "killactive"
      then "hl.dsp.window.close()"
      else if d == "togglefloating"
      then "hl.dsp.window.float({ action = \"toggle\" })"
      else if d == "pseudo"
      then "hl.dsp.window.pseudo()"
      else if d == "fullscreen"
      then "hl.dsp.window.fullscreen()"
      else if d == "workspace"
      then "hl.dsp.focus({ workspace = ${wsArg a} })"
      else if d == "movetoworkspace"
      then "hl.dsp.window.move({ workspace = ${wsArg a} })"
      else if d == "movefocus"
      then "hl.dsp.focus({ direction = ${escapeStr dirMap.${a}} })"
      else if d == "movewindow"
      then "hl.dsp.window.drag()"
      else if d == "resizewindow"
      then "hl.dsp.window.resize()"
      else throw "toHyprlua: unmapped dispatcher '${d}'. Add it to the dispatcher table in hyprlua.nix, or use `{ key = ...; raw = \"hl.dsp....\"; }`.";

  renderConfig = c:
    if c == {}
    then ""
    else "hl.config(${toLua 0 c})";
  ...
  renderBinds = bs:
    concatMapStringsSep "\n" (
      ...
      in "hl.bind(${escapeStr b.key}, ${mkAction b}${flagStr})"
    )
    bs;
