...
let
  ...
  shellCommand =
    if config.cnix.programs.nushell.enable or false then
      "${getExe pkgs.nushell}"
    else if config.cnix.programs.fish.enable or false then
      "${getExe pkgs.fish}"
    else if config.cnix.programs.zsh.enable or false then
      "${getExe pkgs.zsh}"
    else
      null;
  ...
  ghosttyConfig = generators.toKeyValue {
